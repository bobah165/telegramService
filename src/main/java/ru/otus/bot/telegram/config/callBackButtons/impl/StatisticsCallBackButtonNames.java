...
@Component
public class StatisticsCallBackButtonNames implements CallBackButtonNames {
    private final MessageBuilder messageBuilder;
    private final Map<String, String> buttonNames;

    public StatisticsCallBackButtonNames(@Value("#{${buttons.callback.statistics}}") Map<String, String> buttonNames, MessageBuilder messageBuilder) {
    ...
    public ButtonSateWrapper getButtonWrapper() {
        return new ButtonSateWrapper().setButtonNames(buttonNames)
                                      .setBotState(BotState.PROCESSED_STATISTICS);
    }

    @Override
    public CallBackMenu getCallBackMenuName() {
        return CallBackMenu.STATISTIC_MENU;
    }
}
