...
public class MetricsValueValidator implements UserAnswerValidator {
    ...
    public void validateUserAnswer(String usersAnswer, String chatId) {
        ...
        if (!pattern.matcher(usersAnswer).matches()) {
            throw new UserAnswerValidationException(chatId, translator.translateMessage("metrics.wrong.format"));
        }
    }

    @Override
    public BotState getValidatorState() {
        return BotState.METRICS;
    }
}
